...
pub mod test {
    ...
    fn test_round_constants() {
        ...
        assert_eq!(
            ROUND_CONSTANTS[1],
            uint!(7120861356467848435263064379192047478074060781135320967663101236819528304084_U256)
        );
        assert_eq!(
            ROUND_CONSTANTS[2],
            uint!(5024705281721889198577876690145313457398658950011302225525409148828000436681_U256
            )
        );
        assert_eq!(
            ROUND_CONSTANTS[218],
            uint!(2119542016932434047340813757208803962484943912710204325088879681995922344971_U256
            )
        );
    ...
    fn test_mix() {
        ...
        assert_eq!(
            left,
            uint!(8792246410719720074073794355580855662772292438409936688983564419486782556587_U256
            )
        );
        assert_eq!(
            right,
            uint!(7326554092124867281481480523863654579712861994895051796475958890524736238844_U256)
        );
        left += U256::from(2);
        mix(&mut left, &mut right);
        assert_eq!(
            left,
            uint!(19814528709687996974327303300007262407299502847885145507292406548098437687919_U256)
        );
        assert_eq!(
            right,
            uint!(3888906192024793285683241274210746486868893421288515595586335488978789653213_U256)
        );
    }
